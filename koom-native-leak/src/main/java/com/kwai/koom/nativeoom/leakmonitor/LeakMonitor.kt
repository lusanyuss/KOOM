...
object LeakMonitor : LoopMonitor<LeakMonitorConfig>() {
    ...
    private external fun nativeInstallMonitor(
        selectedList: Array<String>, ignoreList: Array<String>, enableLocalSymbolic: Boolean
    ): Boolean
    ...
    override fun call(): LoopState {
        if(monitorConfig.nativeHeapAllocatedThreshold > 0 && Debug.getNativeHeapAllocatedSize() > monitorConfig.nativeHeapAllocatedThreshold) {
            return LoopState.Continue
        }
        
        mutableMapOf<String, LeakRecord>().apply { nativeGetLeakAllocs(this) }.also { AllocationTagLifecycleCallbacks.bindAllocationTag(it) }
            .also { MonitorLog.i(TAG, "LeakRecordMap size: ${it.size}") }.also { monitorConfig.leakListener.onLeak(it.values) }
        return LoopState.Continue
    }
    ...
    override fun startLoop(clearQueue: Boolean, postAtFront: Boolean, delayMillis: Long) {
        ...
        getLoopHandler().post(Runnable {
            ...
            if(!nativeInstallMonitor(
                    monitorConfig.selectedSoList, monitorConfig.ignoredSoList, monitorConfig.enableLocalSymbolic
                )
            ) {
                mIsStart = false

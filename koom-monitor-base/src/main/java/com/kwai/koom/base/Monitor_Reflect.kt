...
fun <T> Class<*>.getStaticFiledValue(filedName: String): T? {
    return runCatching {
        return@runCatching getFiledQuietly(filedName)?.get(null) as T?
    }.getOrNull()
}

fun <T> Any.getFiledValue(filedName: String): T? {
    return runCatching {
        return@runCatching this::class.java.getFiledQuietly(filedName)?.get(this) as T?
    }.getOrNull()
}

fun Class<*>.setStaticFiledValue(filedName: String, filedValue: Any?) {
    runCatching {
        return@runCatching getFiledQuietly(filedName)?.set(null, filedValue)
    }
}

fun Any.setFiledValue(filedName: String, filedValue: Any?) {
    runCatching {
        return@runCatching this::class.java.getFiledQuietly(filedName)?.set(this, filedValue)
    }
}
...
fun Class<*>.getFiledQuietly(filedName: String): Field? {
    return runCatching {
        ...
        while (targetClass != Any::class.java) {
            val targetField = runCatching { targetClass?.getDeclaredField(filedName) }.getOrNull()
...
fun <T> Any.callMethod(
    methodName: String, parameterTypes: Array<Class<*>>? = null, args: Array<Any>? = null
): T? {
    return runCatching {
        val method = this::class.java.getDeclaredMethodQuietly(methodName, parameterTypes) ?: return@runCatching null
...
fun <T> Class<*>.callStaticMethod(
    ...
    return runCatching {
        val method = this.getDeclaredMethodQuietly(methodName, parameterTypes) ?: return@runCatching null
...
fun Class<*>.getDeclaredMethodQuietly(
    filedName: String, parameterTypes: Array<Class<*>>? = null
): Method? {
    return runCatching {
        var targetClass: Class<*>? = this
        while (targetClass != Any::class.java) {

...
fun getRamAvailableSize(context: Context): Long {
    ...
    (context.getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager).also { it.getMemoryInfo(memoryInfo) }
    
    return memoryInfo.availMem
}

fun getCpuCoreCount(): Int {
    return mCpuCoreCount ?: runCatching {
        File("/sys/devices/system/cpu/").listFiles { pathname -> Pattern.matches("cpu[0-9]+", pathname.name) }?.size ?: 0
    }.getOrDefault(Runtime.getRuntime().availableProcessors()).also { mCpuCoreCount = it }
}

fun getCpuMaxFreq(): Double {
    return mCpuMaxFreq ?: runCatching {
        (File("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq").readFirstLine()?.trim()?.toDouble() ?: 0.0) / 1000
...
fun getProcessStatus(): ProcessStatus {
    ...
    runCatching {
        File("/proc/self/status").useLines {
            it.forEach { line ->
                if(processStatus.vssKbSize != 0L && processStatus.rssKbSize != 0L && processStatus.threadsCount != 0L) {
                ...
                when {
                    ...
                    line.startsWith("Threads") -> processStatus.threadsCount = THREADS_REGEX.matchValue(line)
                }
            }
        }
    }
...
class MemInfo(
    @JvmField var totalInKb: Long = 0,
    @JvmField var freeInKb: Long = 0,
    @JvmField var availableInKb: Long = 0,
    @JvmField var IONHeap: Long = 0,
    @JvmField var cmaTotal: Long = 0,
    @JvmField var rate: Float = 0f
)

class JavaHeap(
    @JvmField var max: Long = 0, @JvmField var total: Long = 0, @JvmField var free: Long = 0, @JvmField var used: Long = 0, @JvmField var rate: Float = 0f
)

private fun Regex.matchValue(s: String) = matchEntire(s.trim())?.groupValues?.getOrNull(1)?.toLong() ?: 0L
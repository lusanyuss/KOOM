...
class CommonConfig private constructor( // MonitorManager common properties
    val application: Application,
    
    // Custom FileManager or sharedPreferences
    val rootFileInvoker: (String) -> File, val sharedPreferencesInvoker: (String) -> SharedPreferences, val sharedPreferencesKeysInvoker: (SharedPreferences) -> Set<String>,
    
    // MonitorBuildConfig common properties
    internal val debugMode: Boolean, internal val versionNameInvoker: () -> String,
    
    internal val logger: Logger, internal val log: Log,
    
    // toolbox
    internal val loadSoInvoker: (String) -> Unit, internal val executorServiceInvoker: (() -> ExecutorService)?,
    ...
    class Builder {
        ...
        fun build(): CommonConfig = CommonConfig(application = mApplication,
            
            debugMode = mDebugMode, versionNameInvoker = mVersionNameInvoker,
            
            rootFileInvoker = mRootFileInvoker ?: {
                val rootDir = runCatching { mApplication.getExternalFilesDir("") }.getOrNull()
                
                File(rootDir ?: mApplication.filesDir, "performance/$it").apply { mkdirs() }
            },
            ...
            logger = mLogger ?: object : Logger {}, log = mLog ?: object : Log {},
            
            loadSoInvoker = mLoadSoInvoker ?: { System.loadLibrary(it) }, executorServiceInvoker = mExecutorServiceInvoker,

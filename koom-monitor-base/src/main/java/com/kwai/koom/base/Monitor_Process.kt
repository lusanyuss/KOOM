...
fun getProcessName(): String? {
    return mProcessName ?: getProcessNameByAms()?.also { mProcessName = it } ?: getProcessNameByProc()?.also { mProcessName = it }
}
...
fun getCurrentAbi(): Abi {
    ...
    // Check system api
    "dalvik.system.VMRuntime".toClass()?.callStaticMethod<Any>("getRuntime")?.run { callMethod<Boolean>("is64Bit") }?.also {
            mCurrentAbi = if(it) Abi.ARM64_V8A else Abi.ARMEABI_V7A
            return mCurrentAbi
        }
    
    // Check address size
    "sun.misc.Unsafe".toClass()?.callStaticMethod<Any>("getUnsafe")?.run { callMethod<Int>("addressSize") }?.also {
    ...
    // Check so path
    try {
        getApplication().applicationInfo.nativeLibraryDir.contains("arm64").also { mCurrentAbi = if(it) Abi.ARM64_V8A else Abi.ARMEABI_V7A }
    } catch (e: Throwable) {
        e.printStackTrace()
    }
    return mCurrentAbi
...
private fun getProcessNameByAms(): String? {
    try {
        val activityManager = MonitorManager.getApplication().getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager

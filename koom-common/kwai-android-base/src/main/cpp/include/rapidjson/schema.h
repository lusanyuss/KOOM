...
class Hasher {
 public:
  ...
  Hasher(Allocator *allocator = 0, size_t stackCapacity = kDefaultSize) : stack_(allocator,
																				 stackCapacity) {}
 ...
 public:
  ...
  Schema(SchemaDocumentType *schemaDocument,
	...
	if (dependencies && dependencies->IsObject()) {
	  ...
	  for (ConstMemberIterator itr = dependencies->MemberBegin(); itr != dependencies->MemberEnd();
		   ++itr) {
	...
	if (const ValueType *v = GetMember(value, GetItemsString())) {
	  ...
	  if (v->IsObject()) // List validation
		...
		itemsTuple_ =
			static_cast<const Schema **>(allocator_->Malloc(sizeof(const Schema *) * v->Size()));
		SizeType index = 0;
  ...
  bool StartObject(Context &context) const {
	...
	if (patternProperties_) { // pre-allocate schema array
	  ...
	  context.patternPropertiesSchemas =
		  static_cast<const SchemaType **>(context.factory.MallocState(
...
#define RAPIDJSON_STRING_(name, ...) \
  ...
  RAPIDJSON_STRING_(AdditionalItems,
					'a',
					'd',
					'd',
					'i',
...
#undef RAPIDJSON_STRING_
 ...
 private:
  ...
  static const SchemaType *GetTypeless() {
	static SchemaType
		typeless(0, PointerType(), ValueType(kObjectType).Move(), ValueType(kObjectType).Move(), 0);
	return &typeless;
  }
  ...
  void AssignIfExist(SchemaArray &out,
	...
	if (const ValueType *v = GetMember(value, name)) {
	  if (v->IsArray() && v->Size() > 0) {
		...
		out.count = v->Size();
		out.schemas =
			static_cast<const Schema **>(allocator_->Malloc(out.count * sizeof(const Schema *)));
...
struct TokenHelper {
  RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack &documentStack, SizeType index) {
	*documentStack.template Push<Ch>() = '/';
	char buffer[21];
	size_t length =
		static_cast<size_t>((sizeof(SizeType) == 4 ? u32toa(index, buffer) : u64toa(index, buffer))
			- buffer);
 ...
 private:
  ...
  struct SchemaRefEntry {
	SchemaRefEntry(const PointerType &s,
				   ...
				   Allocator *allocator) : source(s, allocator),
										   target(t, allocator),
										   schema(outSchema) {}
	PointerType source;
	PointerType target;
  ...
  struct SchemaEntry {
	SchemaEntry(const PointerType &p, SchemaType *s, bool o, Allocator *allocator) : pointer(p,
																							 allocator),
																					 schema(s),
																					 owned(o) {}
	~SchemaEntry() {
  ...
  void CreateSchema(const SchemaType **schema,
	...
	if (v.IsObject()) {
	  if (!HandleRefSchema(pointer, schema, v, document)) {
		SchemaType *s = new(allocator_->Malloc(sizeof(SchemaType))) SchemaType(this,
																			   pointer,
																			   v,
																			   document,
																			   allocator_);
		...
		if (i > 0) { // Remote reference, resolve immediately
		  if (remoteProvider_) {
			if (const GenericSchemaDocument
				*remoteDocument = remoteProvider_->GetRemoteDocument(s, i - 1)) {
 ...
 private:
  ...
  bool BeginValue() {
	if (schemaStack_.Empty())
	  ...
	  typename Context::PatternValidatorType
		  patternValidatorType = CurrentContext().valuePatternValidatorType;

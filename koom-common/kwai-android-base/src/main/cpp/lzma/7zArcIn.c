...
void SzBitUi32s_Free(CSzBitUi32s *p, ISzAllocPtr alloc) {
  ISzAlloc_Free(alloc, p->Defs);
  p->Defs = NULL;
  ISzAlloc_Free(alloc, p->Vals);
  p->Vals = NULL;
}
...
static MY_NO_INLINE SRes ReadUi32s(CSzData *sd2,
								   UInt32 numItems,
								   CSzBitUi32s *crcs,
								   ISzAllocPtr alloc) {
  UInt32 i;
...
UInt64 SzAr_GetFolderUnpackSize(const CSzAr *p, UInt32 folderIndex) {
  return p->CoderUnpackSizes[p->FoToCoderUnpackSizes[folderIndex]
	  + p->FoToMainUnpackSizeIndex[folderIndex]];
}
	  ...
	  if (type >= ((UInt32)1 << 8)) {
		...
		switch ((unsigned)type) {
		  ...
		  case k7zIdWinAttrib: {
			...
			*/
		  case k7zIdMTime: RINOK(ReadTime(&p->MTime,
										  numFiles,
										  sd,
										  tempBufs,
										  *numTempBufs,
										  allocMain));
			break;
		  case k7zIdCTime: RINOK(ReadTime(&p->CTime,
										  numFiles,
										  sd,
										  tempBufs,
										  *numTempBufs,
	  ...
	  for (i = 0; i < numFiles; i++, mask >>= 1) {
		...
		if (--remSubStreams == 0) {
		  ...
		  if (numSubStreams == 1 && SzBitWithVals_Check(&p->db.FolderCRCs, i)) {
			...
			crcMask |= mask;
		  } else if (allDigestsDefined
			  || (digestsDefs && SzBitArray_Check(digestsDefs, digestIndex))) {
  ...
  if (res == SZ_OK) {
	...
	if (CrcCalc(buf.data, nextHeaderSizeT) == nextHeaderCRC) {
	  ...
	  if (res == SZ_OK && type == k7zIdEncodedHeader) {
		...
		SzAr_Init(&tempAr);
		res = SzReadAndDecodePackedStreams(inStream,
										   &sd,
										   &tempBuf,
										   1,
										   p->startPosAfterHeader,
										   &tempAr,
										   allocTemp);
		SzAr_Free(&tempAr, allocTemp);

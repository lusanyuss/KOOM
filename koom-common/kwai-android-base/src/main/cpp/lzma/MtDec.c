...
static SRes MtDec_Progress_GetError_Spec(CMtDec *p,
										 UInt64 inSize,
										 UInt64 outSize,
  ...
  if (p->mtProgress.res == SZ_OK && p->mtProgress.progress)
	if (ICompressProgress_Progress(p->mtProgress.progress,
								   p->mtProgress.totalInSize,
								   p->mtProgress.totalOutSize) != SZ_OK)
	...
	if (res == SZ_OK && !wasInterrupted) {
	  // if (p->inStream)
	  {
		...
		for (;;) {
		  if (!link) {
			link =
				(CMtDecBufLink *)ISzAlloc_Alloc(p->alloc, MTDEC__LINK_DATA_OFFSET + p->inBufSize);
			if (!link) {
			  finish = True;
...

			{
			  ...
			  if (parse.state != MTDEC_PARSE_CONTINUE || finish) {
				...
				if (parse.state == MTDEC_PARSE_END) {
				  ...
				  if (crossSize != 0)
					memcpy(data + parse.srcSize,
						   parseData + parse.srcSize,
						   size - parse.srcSize); // we need all data
...

	{
	  ...
	  if (needWrite) {
		...
		res = p->mtCallback->Write(p->mtCallbackObject, t->index,
								   res == SZ_OK && needWriteToStream
									   && !wasInterrupted, // needWrite
...
SRes MtDec_Code(CMtDec *p) {
...

  {
	...
	if (wres == 0) {
	  wres = Event_Set(&nextThread->canWrite);
	  if (wres == 0) {
		wres = Event_Set(&nextThread->canRead);
		if (wres == 0) {
		  wres = ThreadFunc(nextThread);
		  if (wres != 0) {

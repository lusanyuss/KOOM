  ...
  for (;;) {
	...
	for (;;) {
...

	  {
		...
		{
		  UInt32 *heads =
			  mt->hashBuf + ((numProcessedBlocks++) & kMtHashNumBlocksMask) * kMtHashBlockSize;
		  ...
		  if (num >= mf->numHashBytes) {
			...
			mt->GetHeadsFunc(mf->buffer,
							 mf->pos,
							 mf->hash + mf->fixedHashSize,
							 mf->hashMask,
							 heads + 2,
							 num,
							 mf->crc);
...
static void MatchFinderMt_GetNextBlock_Hash(CMatchFinderMt *p) {
  ...
  p->hashBufPosLimit = p->hashBufPos =
	  ((p->hashSync.numProcessedBlocks - 1) & kMtHashNumBlocksMask) * kMtHashBlockSize;
...
MY_NO_INLINE
static UInt32 *GetMatchesSpecN(UInt32 lenLimit,
							   UInt32 pos,
							   const Byte *cur,
							   CLzRef *son,
	...
	*/
	if (cutValue == 0 || delta >= _cyclicBufferSize) {
	  *ptr0 = *ptr1 = kEmptyHashValue;
	  ...
	  for (;;) {
		{
		  CLzRef *pair = son + ((size_t)(
			  _cyclicBufferPos - delta + ((_cyclicBufferPos < delta) ? _cyclicBufferSize : 0))
			  << 1);
...
static void BtGetMatches(CMatchFinderMt *p, UInt32 *distances) {
  ...
  while (curPos < limit) {
	...
	}
	{
	  ...
	  {
		UInt32 posRes;
		curPos = (UInt32)(GetMatchesSpecN(lenLimit,
										  pos,
										  p->buffer,
										  p->son,
										  cyclicBufferPos,
...
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE HashThreadFunc2(void *p) {
...
SRes MatchFinderMt_Create(CMatchFinderMt *p, UInt32 historySize, UInt32 keepAddBufferBefore,
  ...
  if (!p->hashBuf) {
	p->hashBuf =
		(UInt32 *)ISzAlloc_Alloc(alloc, (kHashBufferSize + kBtBufferSize) * sizeof(UInt32));
	if (!p->hashBuf)
	  return SZ_ERROR_MEM;
...
void MatchFinderMt_CreateVTable(CMatchFinderMt *p, IMatchFinder *vTable) {
  ...
  switch (p->MatchFinder->numHashBytes) {
	case 2:p->GetHeadsFunc = GetHeads2;
	  p->MixMatchesFunc = (Mf_Mix_Matches)NULL;
	  vTable->Skip = (Mf_Skip_Func)MatchFinderMt0_Skip;
	  vTable->GetMatches = (Mf_GetMatches_Func)MatchFinderMt2_GetMatches;
	  break;
	case 3:p->GetHeadsFunc = GetHeads3;

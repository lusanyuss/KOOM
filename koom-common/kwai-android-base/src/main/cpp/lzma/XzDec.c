...
static SizeT BraState_Filter(void *pp, Byte *data, SizeT size) {
  ...
  switch (p->methodId) {
	case XZ_ID_Delta:
	  ...
	  break;
	case XZ_ID_X86:size = x86_Convert(data, size, p->ip, &p->x86State, p->encodeMode);
	  break;
	CASE_BRA_CONV(PPC)
	CASE_BRA_CONV(IA64)
	CASE_BRA_CONV(ARM)
...
static SRes Lzma2State_Code2(void *pp, Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
  ...
  if (spec->outBufMode) {
	...
	res = Lzma2Dec_DecodeToDic(&spec->decoder,
							   dicLimit,
							   src,
							   srcLen,
	...
	*destLen = spec->decoder.decoder.dicPos - dicPos;
	...
	res = Lzma2Dec_DecodeToBuf(&spec->decoder,
							   dest,
							   destLen,
							   src,
							   srcLen,
...
static SRes Lzma2State_SetFromMethod(IStateCoder *p,
									 Byte *outBuf,
									 size_t outBufSize,
									 ISzAllocPtr alloc) {
...
static SRes MixCoder_SetFromMethod(CMixCoder *p,
								   unsigned coderIndex,
								   UInt64 methodId,
								   Byte *outBuf,
								   size_t outBufSize) {
...
static SRes MixCoder_ResetFromMethod(CMixCoder *p,
									 unsigned coderIndex,
									 UInt64 methodId,
									 Byte *outBuf,
									 size_t outBufSize) {
...
SRes XzBlock_Parse(CXzBlock *p, const Byte *header) {
  ...
  if (XzBlock_HasUnpackSize(p)) READ_VARINT_AND_CHECK(header, pos, headerSize, &p->unpackSize);
...
static SRes XzDecMix_Init(CMixCoder *p, const CXzBlock *block, Byte *outBuf, size_t outBufSize) {
  ...
  if (needReInit) {
	MixCoder_Free(p);
	for (i = 0; i < numFilters; i++) {
	  RINOK(MixCoder_SetFromMethod(p,
								   i,
								   block->filters[numFilters - 1 - i].id,
								   outBuf,
								   outBufSize));
	}

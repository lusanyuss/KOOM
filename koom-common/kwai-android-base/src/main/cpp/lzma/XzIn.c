...
UInt64 Xz_GetUnpackSize(const CXzStream *p) {
  UInt64 size = 0;
  size_t i;
  for (i = 0; i < p->numBlocks; i++) ADD_SIZE_CHECK(size, p->blocks[i].unpackSize);
  return size;
}
...
UInt64 Xz_GetPackSize(const CXzStream *p) {
  UInt64 size = 0;
  size_t i;
  for (i = 0; i < p->numBlocks; i++) ADD_SIZE_CHECK(size,
													(p->blocks[i].totalSize + 3) & ~(UInt64)3);
  return size;
}
...
static SRes LookInStream_SeekRead_ForArc(ILookInStream *stream,
										 UInt64 offset,
										 void *buf,
										 size_t size) {
...
static SRes Xz_ReadBackward(CXzStream *p,
...
UInt64 Xzs_GetUnpackSize(const CXzs *p) {
  UInt64 size = 0;
  size_t i;
  for (i = 0; i < p->num; i++) ADD_SIZE_CHECK(size, Xz_GetUnpackSize(&p->streams[i]));
  return size;
}
...
SRes Xzs_ReadBackward(CXzs *p,
  ...
  *startOffset = endOffset;
  for (;;) {
	...
	if (progress
		&& ICompressProgress_Progress(progress, endOffset - *startOffset, (UInt64)(Int64)-1)
			!= SZ_OK)

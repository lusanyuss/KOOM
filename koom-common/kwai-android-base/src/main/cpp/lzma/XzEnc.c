...
static SRes XzEncIndex_PreAlloc(CXzEncIndex *p,
								UInt64 numBlocks,
								UInt64 unpackSize,
								UInt64 totalSize,
								ISzAllocPtr alloc) {
...
static SRes XzEncIndex_AddIndexRecord(CXzEncIndex *p,
									  UInt64 unpackSize,
									  UInt64 totalSize,
									  ISzAllocPtr alloc) {
  Byte buf[32];
  ...
  if (p->blockSize == XZ_PROPS__BLOCK_SIZE__SOLID) {
	...
	if (p->blockSize == LZMA2_ENC_PROPS__BLOCK_SIZE__AUTO) {
	  ...
	  if (lzma2->blockSize == LZMA2_ENC_PROPS__BLOCK_SIZE__SOLID) {
		...
		if (lzma2->lzmaProps.reduceSize > tp.blockSize
			&& tp.blockSize != LZMA2_ENC_PROPS__BLOCK_SIZE__SOLID)
	  ...
	  else if (lzma2->blockSize > p->blockSize
		  && lzma2->blockSize != LZMA2_ENC_PROPS__BLOCK_SIZE__SOLID)
...
static SRes Xz_CompressBlock(
  ...
  */
  ...
  }
  {
	...
	RINOK(WriteBytes(&seqSizeOutStream.vt,
					 buf + (4 - padSize),
...
static SRes CompressProgress_XzEncOffset_Progress(const ICompressProgress *pp,
												  UInt64 inSize,
												  UInt64 outSize) {
  const CCompressProgress_XzEncOffset
	  *p = CONTAINER_FROM_VTBL(pp, CCompressProgress_XzEncOffset, vt);
  inSize += p->inOffset;
  outSize += p->outOffset;
...
SRes XzEnc_Encode(CXzEncHandle pp,
				  ISeqOutStream *outStream,
				  ISeqInStream *inStream,
				  ICompressProgress *progress) {
  CXzEnc *p = (CXzEnc *)pp;
  ...
  XzEncIndex_Init(&p->xzIndex);
  {
	...
	RINOK(XzEncIndex_PreAlloc(&p->xzIndex,
							  numBlocks,
							  blockSize,
	...
	for (;;) {
	  ...
	  */
...

	  {
		...
		RINOK(XzEncIndex_AddIndexRecord(&p->xzIndex,
										blockSizes.unpackSize,
										blockSizes.totalSize,
										p->alloc));

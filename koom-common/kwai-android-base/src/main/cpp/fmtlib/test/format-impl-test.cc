...
struct value_extractor {
  ...
  template<typename U> FMT_NORETURN T
  operator()(U) {
...
format_to(buffer,
"garbage");
fmt::detail::format_error_code(buffer,
42, "test");
EXPECT_EQ("test: " + msg,
to_string(buffer)
);
...
{
fmt::memory_buffer buffer;
std::string prefix(fmt::inline_buffer_size
- msg.
size()
- sep.
size()
...
int codes[] = {42, -1};
for (
size_t i = 0, n = sizeof(codes) / sizeof(*codes);
i<n;
++i) {

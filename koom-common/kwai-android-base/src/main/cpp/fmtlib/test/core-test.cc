...
EXPECT_FALSE(std::is_move_constructible<buffer < char>>
::value);
#  if !FMT_MSC_VER
// std::is_move_assignable is broken in MSVC2013.
EXPECT_FALSE(std::is_move_assignable<buffer < char>>
::value);
#  endif
}
#endif
...
struct formatter<custom_type> {
  ...
  auto format(const custom_type &p, FormatContext &ctx) ->
  decltype(ctx
  .
  out()
  ) {
...
char str[]{"1234567890"};
store.
push_back(fmt::arg("a1", str)
);
store.
push_back(fmt::arg("a2", std::cref(str))
);
str[0] = 'X';
...
custom_type c{};
store.
push_back(fmt::arg("c1", c)
);
++c.
i;
store.
push_back(fmt::arg("c2", c)
);
++c.
i;
store.
push_back(fmt::arg("c_ref", std::cref(c))
);
++c.
i;
...
struct formatter<copy_throwable> {
  ...
  auto format(copy_throwable, format_context &ctx) ->
  decltype(ctx
  .
  out()
  ) {
...
struct formatter<enabled_formatter> {
  ...
  auto format(enabled_formatter, format_context &ctx) ->
  decltype(ctx
  .
  out()
  ) {
...
struct formatter<convertible_to_int> {
  ...
  auto format(convertible_to_int, format_context &ctx) ->
  decltype(ctx
  .
  out()
  ) {
...
EXPECT_TRUE(fmt::detail::is_string<fmt::basic_string_view < TypeParam>>
::value);
EXPECT_TRUE(

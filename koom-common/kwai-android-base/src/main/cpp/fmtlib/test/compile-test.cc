...
fmt::format_to(fmt::detail::make_checked(buffer, 32), prepared,
2);
EXPECT_EQ(std::string("42"), buffer
);
wchar_t wbuffer[32] = {0};
const auto wprepared = fmt::detail::compile<int>(L"4{}");
fmt::format_to(fmt::detail::make_checked(wbuffer, 32), wprepared,
2);
...
struct formatter<formattable> : formatter<const char *> {
  ...
  auto format(formattable, FormatContext &ctx) ->
  decltype(ctx
  .
  out()
  ) {

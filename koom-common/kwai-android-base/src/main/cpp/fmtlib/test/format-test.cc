...
EXPECT_EQ(fmt::detail::bit_cast<uint64_t>(s),
42ull);
...
EXPECT_FALSE(fmt::detail::is_output_iterator<std::list < char>>
::value);
EXPECT_TRUE(
...
typedef allocator_ref<std::allocator < char>>
TestAllocator;
...
EXPECT_EQ(L"\xD801\xDC37", fmt::detail::utf8_to_utf16("𐐷").
str()
);
...
EXPECT_EQ(L"123456", fmt::detail::utf8_to_utf16("123456").
str()
);
}
...
fmt::format_system_error(message, EDOM,
"test");
...
fmt::format_system_error(out, EDOM,
"test error");
out.push_back('\n');
...
EXPECT_EQ("0x0", format("{0}", static_cast
<void *>(nullptr)
));
...
class QString {
 public:
  ...
  FMT_NOEXCEPT { return s_->data(); }
  int size() const
  FMT_NOEXCEPT { return static_cast<int>(s_->size()); }

 private:
  std::shared_ptr<std::wstring> s_;
};

fmt::basic_string_view<wchar_t> to_string_view(const QString &s)
FMT_NOEXCEPT {
return {
s.
utf16(),
...
L"42");
EXPECT_EQ(fmt::format(QString(L"{}"), QString(L"42")),
L"42");
}
...
struct formatter<Date> {
  ...
  auto format(const Date &d, format_context &ctx) ->
  decltype(ctx
  .
  out()
  ) {
...
args.
emplace_back(fmt::detail::make_arg<ctx>(42)
);
args.
emplace_back(fmt::detail::make_arg<ctx>("abc1")
);
args.
emplace_back(fmt::detail::make_arg<ctx>(1.5f)
);
...
42));
EXPECT_EQ("foo",
fmt::format(FMT_STRING("{}"), string_like()
));
(void)
with_null;
(void)
...
EXPECT_EQ("42",
fmt::format(FMT_STRING("{}"), const_answer
));
}
...
struct formatter<variant> : dynamic_formatter<> {
  auto format(variant value, format_context &ctx) ->
  decltype(ctx
  .
  out()
  ) {
...
namespace detail {
struct foo {};
template<typename, typename OutputIt>
void write(OutputIt, foo) = delete;
}  // namespace detail
}  // namespace fmt
}  // namespace adl_test
...
EXPECT_EQ (fmt::format(fmt::emphasis::bold,
"bold error"),
...
EXPECT_EQ(fmt::detail::count_code_points(result),
4);
EXPECT_EQ(result
.
size(),

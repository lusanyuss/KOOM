...
ASSERT_NE(test_level_one_(1, 2, 3, 4, VerifyLevelBacktrace, nullptr),
0);
}
...
ASSERT_NE(test_level_one_(1, 2, 3, 4, VerifyLevelIgnoreFrames, nullptr),
0);
}

TEST_F(BacktraceTest, local_max_trace
) {
ASSERT_NE(test_recursive_call_(MAX_BACKTRACE_FRAMES + 10, VerifyMaxBacktrace, nullptr),
0);
}
...
ASSERT_NE(test_level_one_(1, 2, 3, 4, nullptr, nullptr),
0);
_exit(1);
}
...
ASSERT_NE(test_recursive_call_(MAX_BACKTRACE_FRAMES + 10, nullptr, nullptr),
0);
_exit(1);
}
...
pthread_t thread;
ASSERT_TRUE(pthread_create(&thread, &attr, PtraceThreadLevelRun, nullptr)
== 0);
}
ASSERT_NE(test_level_one_(1, 2, 3, 4, nullptr, nullptr),
0);
...
.
size(),
static_cast<size_t>(NUM_PTRACE_THREADS + 1));

ASSERT_TRUE(ptrace(PTRACE_ATTACH, pid, 0, 0)
== 0);
WaitForStop(pid);
for (
std::vector<int>::const_iterator it = threads.begin();
it != threads.
end();
++it) {
...
ASSERT_NE(test_level_one_(1, 2, 3, 4, VerifyLevelThread, nullptr),
0);
}
...
ASSERT_NE(test_recursive_call_(MAX_BACKTRACE_FRAMES + 10, VerifyMaxThread, nullptr),
0);
}
...
pthread_t thread;
ASSERT_TRUE(pthread_create(&thread, &attr, ThreadLevelRun, &thread_data)
== 0);
...
ASSERT_TRUE(sigaction(THREAD_SIGNAL, nullptr, &cur_action)
== 0);
...
ASSERT_TRUE(sigaction(THREAD_SIGNAL, nullptr, &new_action)
== 0);
EXPECT_EQ(cur_action
.sa_sigaction, new_action.sa_sigaction);
...
thread_t thread_data = {0, 0, 0, nullptr};
pthread_t thread;
ASSERT_TRUE(pthread_create(&thread, &attr, ThreadMaxRun, &thread_data)
== 0);
...
ASSERT_TRUE(backtrace
->Unwind(0));
ASSERT_EQ(BACKTRACE_UNWIND_ERROR_EXCEED_MAX_FRAMES_LIMIT, backtrace
->
GetError()
.error_code);

VerifyMaxDump(backtrace
.
get()
);
...
pthread_t thread;
thread_t thread_data = {0, 0, 0, nullptr};
ASSERT_TRUE(pthread_create(&thread, &attr, ThreadReadTest, &thread_data)
== 0);
...
ASSERT_TRUE(ptrace(PTRACE_DETACH, pid, 0, 0)
== 0);
}
if ((
NanoTime()
...
struct stat buf;
ASSERT_TRUE(stat(tmp_so_name.c_str(), &buf)
!= -1);
...
!= MAP_FAILED);
close(fd);
ASSERT_TRUE(unlink(tmp_so_name.c_str())
!= -1);
...
map_size += reinterpret_cast
<uint64_t>(map);
std::string last_func;
for (
uint64_t read_addr = reinterpret_cast<uint64_t>(map);
read_addr<map_size;
read_addr += 4) {
uint64_t offset;
...
ASSERT_TRUE(munmap(map, map_size - reinterpret_cast<uint64_t>(map))
== 0);
...
backtrace->Read(reinterpret_cast
<uint64_t>(&g_ready),
reinterpret_cast<uint8_t*>(&read_addr), sizeof(g_ready)));
if (read_addr) {
...
size_t frame_num;
if (
FindFuncFrameInBacktrace(backtrace
.
get(),
reinterpret_cast
<uint64_t>(test_func),
...
frame_num != 0) {
VerifyUnreadableElfFrame(backtrace
.
get(),
reinterpret_cast
<uint64_t>(test_func), frame_num
);
done = true;
}
...
ASSERT_TRUE(done)
<< "Test function never found in unwind.";
}
...
->Unwind(0));
ASSERT_EQ(BACKTRACE_UNWIND_ERROR_THREAD_DOESNT_EXIST, backtrace
->
GetError()
.error_code);
}
...
uint64_t offset;
ASSERT_NE(std::string(""), backtrace
->
GetFunctionName(cur_func_offset, &offset
));
}
...
thread_t thread_data = {0, 0, 0, nullptr};
pthread_t thread;
ASSERT_TRUE(pthread_create(&thread, nullptr, ThreadLevelRun, &thread_data)
== 0);
...
CheckForLeak(BACKTRACE_CURRENT_PROCESS, thread_data
.tid);

// Tell the thread to exit its infinite loop.
android_atomic_acquire_store(0, &thread_data.state);

ASSERT_TRUE(pthread_join(thread, nullptr)
== 0);
}

  ...
  for (size_t i = 1; i < cie->augmentation_string.size(); i++) {
	switch (cie->augmentation_string[i]) {
	  ...
	  case 'R':
		...
		break;
	  case 'S':cie->is_signal_frame = true;
		break;
	}
  }
  return true;
...
bool DwarfSectionImpl<AddressType>::EvalRegister(const DwarfLocation *loc, uint32_t reg,
  ...
  switch (loc->type) {
	case DWARF_LOCATION_OFFSET:
	  if (!regular_memory->ReadFully(eval_info->cfa + loc->values[0],
									 reg_ptr,
									 sizeof(AddressType))) {
		last_error_.code = DWARF_ERROR_MEMORY_INVALID;
	  ...
	  break;
	case DWARF_LOCATION_VAL_OFFSET:*reg_ptr = eval_info->cfa + loc->values[0];
	  break;
	...
	case DWARF_LOCATION_UNDEFINED:
	  ...
	  break;
	case DWARF_LOCATION_PSEUDO_REGISTER:last_error_.code = DWARF_ERROR_ILLEGAL_VALUE;
	  return false;
	default:break;
  }
...
bool DwarfSectionImpl<AddressType>::Eval(const DwarfCie *cie, Memory *regular_memory,
  ...
  switch (loc->type) {
	...
	default:last_error_.code = DWARF_ERROR_ILLEGAL_VALUE;
	  return false;
  }
...
template
class DwarfSectionImpl<uint32_t>;
template
class DwarfSectionImpl<uint64_t>;

// Explicitly instantiate DwarfDebugFrame
template
class DwarfDebugFrame<uint32_t>;
template
class DwarfDebugFrame<uint64_t>;

// Explicitly instantiate DwarfEhFrame
template
class DwarfEhFrame<uint32_t>;
template
class DwarfEhFrame<uint64_t>;

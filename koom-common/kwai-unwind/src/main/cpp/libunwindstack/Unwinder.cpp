...
KWAI_EXPORT void Unwinder::Unwind(const std::vector<std::string> *initial_map_names_to_skip,
...
KWAI_EXPORT std::string Unwinder::FormatFrame(const FrameData &frame) const {
  std::string data;
...
KWAI_EXPORT std::string Unwinder::FormatFrame(size_t frame_num) const {
...
KWAI_EXPORT bool UnwinderFromPid::Init() {
  CHECK(arch_ != ARCH_UNKNOWN);
  if (initted_) {
	return true;
  }
...
KWAI_EXPORT FrameData Unwinder::BuildFrameFromPcOnly(uint64_t pc) {
  return BuildFrameFromPcOnly(pc, arch_, maps_, jit_debug_, process_memory_, resolve_names_);
}

KWAI_EXPORT Unwinder::Unwinder(size_t max_frames) : max_frames_(max_frames) {
  frames_.reserve(max_frames);
}

KWAI_EXPORT Unwinder::Unwinder(size_t max_frames, ArchEnum arch)
	: max_frames_(max_frames), arch_(arch) {
  frames_.reserve(max_frames);
}

KWAI_EXPORT UnwinderFromPid::UnwinderFromPid(size_t max_frames, pid_t pid)
	: Unwinder(max_frames), pid_(pid) {}

KWAI_EXPORT UnwinderFromPid::UnwinderFromPid(size_t max_frames, pid_t pid, ArchEnum arch)
	: Unwinder(max_frames, arch), pid_(pid) {}

KWAI_EXPORT UnwinderFromPid::~UnwinderFromPid() = default;

KWAI_EXPORT Unwinder::~Unwinder() = default;

...
bool CreateElfFromMemory(std::shared_ptr<unwindstack::Memory> &memory, map_info_t *info) {
  std::string cur_name;
  if (info->name.empty()) {
	cur_name = android::base::StringPrintf("anonymous_%"
	PRIx64, info->start);
  } else {
	cur_name = android::base::StringPrintf("%s_%"
	PRIx64, basename(info->name.c_str()), info->start);
  }
  ...
  if (bytes == 0) {
	printf("Cannot read data from address %"
	PRIx64
...
int SaveData(pid_t pid) {
  ...
  for (auto &element : sorted_maps) {
	...
	" %s %"
	PRIx64
	" 00:00 0", map.start, map.end, perms,
		map.offset);
	if (!map.name.empty()) {

...
static size_t ProcessVmRead(pid_t pid, uint64_t remote_src, void *dst, size_t len) {
  ...
  while (len > 0) {
	...
	ssize_t rc = kwai_process_vm_readv(pid, &dst_iov, 1, src_iovs, iovecs_used, 0);
	if (rc == -1) {
	  return total_read;
	}
	total_read += rc;
...
KWAI_EXPORT bool Memory::ReadFully(uint64_t addr, void *dst, size_t size) {
...
MemoryFileAtOffset::~MemoryFileAtOffset() { Clear(); }
...
long MemoryRemote::ReadTag(uint64_t) { return -1; }

long MemoryLocal::ReadTag(uint64_t) { return -1; }
#endif

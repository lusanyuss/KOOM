...
bool DwarfCfa<AddressType>::GetLocationInfo(uint64_t pc, uint64_t start_offset, uint64_t end_offset,
  ...
  while (true) {
	...
	switch (cfa_value >> 6) {
	  case 1:cur_pc_ += cfa_low * fde_->cie->code_alignment_factor;
		break;
	  case 2: {
		uint64_t offset;
...
std::string DwarfCfa<AddressType>::GetOperandString(uint8_t operand, uint64_t value,
  ...
  std::string string;
  switch (operand) {
	case DwarfCfaInfo::DWARF_DISPLAY_REGISTER:string = " register(" + std::to_string(value) + ")";
	  break;
	case DwarfCfaInfo::DWARF_DISPLAY_SIGNED_NUMBER:string += " "
		  + std::to_string(static_cast<SignedType>(value));
	  break;
	case DwarfCfaInfo::DWARF_DISPLAY_ADVANCE_LOC:*cur_pc += value;
	  FALLTHROUGH_INTENDED;
	  // Fall through to log the value.
	case DwarfCfaInfo::DWARF_DISPLAY_NUMBER:string += " " + std::to_string(value);
	  break;
	case DwarfCfaInfo::DWARF_DISPLAY_SET_LOC:*cur_pc = value;
	  FALLTHROUGH_INTENDED;
	  // Fall through to log the value.
  ...
  while ((cfa_offset = memory_->cur_offset()) < end_offset && cur_pc <= pc) {
	...
	switch (cfa_value >> 6) {
	  case 0:
		...
		break;
	  case 1:log(indent, "DW_CFA_advance_loc %d", cfa_low);
	  ...
	  case 2:
		...
		break;
	  case 3:log(indent, "DW_CFA_restore register(%d)", cfa_low);
		log(indent, "Raw Data: 0x%02x", cfa_value);
		break;
	}
	if (cur_pc != old_pc) {
...
template
class DwarfCfa<uint32_t>;
template
class DwarfCfa<uint64_t>;

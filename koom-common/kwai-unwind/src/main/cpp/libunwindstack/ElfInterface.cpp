...
void ElfInterface::ReadProgramHeaders(const EhdrType &ehdr, int64_t *load_bias) {
  ...
  for (size_t i = 0; i < ehdr.e_phnum; i++, offset += ehdr.e_phentsize) {
	...
	switch (phdr.p_type) {
	  ...
	  case PT_DYNAMIC:dynamic_offset_ = phdr.p_offset;
	  ...
	  default:HandleUnknownType(phdr.p_type, phdr.p_offset, phdr.p_filesz);
		break;
	}
  }
}
...
template<typename SymType>
bool ElfInterface::GetGlobalVariableWithTemplate(const std::string &name,
...
bool ElfInterface::Step(uint64_t pc, Regs *regs, Memory *process_memory, bool *finished,
  ...
  switch (code) {
	case DWARF_ERROR_NONE:last_error_.code = ERROR_NONE;
	  break;

	case DWARF_ERROR_MEMORY_INVALID:last_error_.code = ERROR_MEMORY_INVALID;
	...
	case DWARF_ERROR_NO_FDES:last_error_.code = ERROR_UNWIND_INFO;
	  break;

	case DWARF_ERROR_NOT_IMPLEMENTED:
	case DWARF_ERROR_UNSUPPORTED_VERSION:last_error_.code = ERROR_UNSUPPORTED;
	  break;
  }
  return false;
}
...
template bool ElfInterface::GetGlobalVariableWithTemplate<Elf32_Sym>(const std::string &,
																	 uint64_t *);
template bool ElfInterface::GetGlobalVariableWithTemplate<Elf64_Sym>(const std::string &,
																	 uint64_t *);

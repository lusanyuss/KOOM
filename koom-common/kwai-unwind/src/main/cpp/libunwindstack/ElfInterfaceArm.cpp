...
bool ElfInterfaceArm::StepExidx(uint64_t pc, Regs *regs, Memory *process_memory, bool *finished) {
  ...
  if (!return_value) {
	switch (arm.status()) {
	  ...
	  case ARM_STATUS_FINISH:last_error_.code = ERROR_NONE;
		break;
	  ...
	  case ARM_STATUS_INVALID_PERSONALITY:last_error_.code = ERROR_UNWIND_INFO;
		break;

	  case ARM_STATUS_READ_FAILED:last_error_.code = ERROR_MEMORY_INVALID;

...
import kshark.RandomAccessHprofReader.Companion.openReaderFor
import okio.Buffer
...
class RandomAccessHprofReader private constructor(
    private val source: RandomAccessSource, hprofHeader: HprofHeader
) : Closeable {
     ...
     */
    fun <T> readRecord(
        recordPosition: Long, recordSize: Long, withRecordReader: HprofRecordReader.() -> T
    ): T {
    ...
    companion object {
        ...
        fun openReaderFor(
            hprofSourceProvider: RandomAccessSourceProvider, hprofHeader: HprofHeader = hprofSourceProvider.openRandomAccessSource().use { HprofHeader.parseHeaderOf(it.asStreamingSource()) }
        ): RandomAccessHprofReader {

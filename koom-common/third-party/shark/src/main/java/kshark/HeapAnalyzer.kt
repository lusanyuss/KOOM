...
import kshark.OnAnalysisProgressListener.Step.*
import kshark.internal.*
...
class HeapAnalyzer constructor(
    ...
    public class FindLeakInput(
        val graph: HeapGraph, val referenceMatchers: List<ReferenceMatcher>, val computeRetainedHeapSize: Boolean, val objectInspectors: List<ObjectInspector>
    )
    ...
    fun analyze(
        ...
        if(!heapDumpFile.exists()) {
            ...
            return HeapAnalysisFailure(
                heapDumpFile = heapDumpFile, createdAtTimeMillis = System.currentTimeMillis(), analysisDurationMillis = since(analysisStartNanoTime), exception = HeapAnalysisException(exception)
            )
        }
        ...
        return try {
            ...
            sourceProvider.openHeapGraph(proguardMapping).use { graph ->
                ...
                val randomAccessStats =
                    "RandomAccess[" + "bytes=${sourceProvider.randomAccessByteReads}," + "reads=${sourceProvider.randomAccessReadCount}," + "travel=${sourceProvider.randomAccessByteTravel}," + "range=${sourceProvider.byteTravelRange}," + "size=${heapDumpFile.length()}" + "]"
    ...
    private fun FindLeakInput.analyzeGraph(
        metadataExtractor: MetadataExtractor, leakingObjectFinder: LeakingObjectFinder, heapDumpFile: File, analysisStartNanoTime: Long
    ): HeapAnalysisSuccess {
    ...
    data class LeaksAndUnreachableObjects(
        val applicationLeaks: List<ApplicationLeak>, val libraryLeaks: List<LibraryLeak>, val unreachableObjects: List<LeakTraceObject>
    )
    
    public fun FindLeakInput.findLeaks(leakingObjectIds: Set<Long>): LeaksAndUnreachableObjects {
    ...
    private fun FindLeakInput.findUnreachableObjects(
        pathFindingResults: PathFindingResults, leakingObjectIds: Set<Long>
    ): List<LeakTraceObject> {
        val reachableLeakingObjectIds = pathFindingResults.pathsToLeakingObjects.map { it.objectId }.toSet()
    ...
    internal sealed class TrieNode {
        ...
        class LeafNode(
            override val objectId: Long, val pathNode: ReferencePathNode
        ) : TrieNode()
    }
    ...
    private fun deduplicateShortestPaths(
        ...
        inputPathResults.forEach { pathNode -> // Go through the linked list of nodes and build the reverse list of instances from
            // root to leaking.
        ...
        if(outputPathResults.size != inputPathResults.size) {
            SharkLog.d {
                "Found ${inputPathResults.size} paths to retained objects," + " down to ${outputPathResults.size} after removing duplicated paths"
            }
        } else {
    ...
    private fun updateTrie(
        pathNode: ReferencePathNode, path: List<Long>, pathIndex: Int, parentNode: ParentNode
    ) {
    ...
    private fun findResultsInTrie(
        parentNode: ParentNode, outputPathResults: MutableList<ReferencePathNode>
    ) {
    ...
    internal class ShortestPath(
        val root: RootNode, val childPath: List<ChildNode>
    ) {
        fun asList() = listOf(root) + childPath
    }
    
    private fun FindLeakInput.buildLeakTraces(
        shortestPaths: List<ShortestPath>, inspectedObjectsByPath: List<List<InspectedObject>>, retainedSizes: Map<Long, Pair<Int, Int>>?
        ...
        shortestPaths.forEachIndexed { pathIndex, shortestPath ->
            ...
            val leakTrace = LeakTrace(
                gcRootType = GcRootType.fromGcRoot(shortestPath.root.gcRoot), referencePath = referencePath, leakingObject = leakTraceObjects.last()
            )
    ...
    private fun FindLeakInput.inspectObjects(shortestPaths: List<ShortestPath>): List<List<InspectedObject>> {
        ...
        val leakReportersByPath = shortestPaths.map { path ->
            ...
            pathList.mapIndexed { index, node ->
    ...
    private fun FindLeakInput.computeRetainedSizes(
        inspectedObjectsByPath: List<List<InspectedObject>>, dominatorTree: DominatorTree
    ): Map<Long, Pair<Int, Int>>? {
        val nodeObjectIds = inspectedObjectsByPath.flatMap { inspectedObjects ->
            inspectedObjects.filter { it.leakingStatus == UNKNOWN || it.leakingStatus == LEAKING }.map { it.heapObject.objectId }
        }.toSet()
    ...
    private fun buildLeakTraceObjects(
        inspectedObjects: List<InspectedObject>, retainedSizes: Map<Long, Pair<Int, Int>>?
    ): List<LeakTraceObject> {
    ...
    private fun FindLeakInput.buildReferencePath(
        shortestChildPath: List<ChildNode>, leakTraceObjects: List<LeakTraceObject>
    ): List<LeakTraceReference> {
        return shortestChildPath.mapIndexed { index, childNode ->
            LeakTraceReference(
                originObject = leakTraceObjects[index], referenceType = childNode.refFromParentType, owningClassName = if(childNode.owningClassId != 0L) {
    ...
    internal class InspectedObject(
        val heapObject: HeapObject, val leakingStatus: LeakingStatus, val leakingStatusReason: String, val labels: MutableSet<String>
    )
    ...
    private fun computeLeakStatuses(leakReporters: List<ObjectReporter>): List<InspectedObject> {
        ...
        for ((index, reporter) in leakReporters.withIndex()) {
            val resolvedStatusPair = resolveStatus(reporter, leakingWins = index == lastElementIndex).let { statusPair ->
                if(index == lastElementIndex) { // The last element should always be leaking.
            ...
            if(leakStatus == NOT_LEAKING) {
                lastNotLeakingElementIndex = index // Reset firstLeakingElementIndex so that we never have
        ...
        if(firstLeakingElementIndex < lastElementIndex - 1) { // We already know the status of firstLeakingElementIndex and lastElementIndex
    ...
    private fun resolveStatus(
        reporter: ObjectReporter, leakingWins: Boolean
        ...
        if(leakingReasons.isNotEmpty()) {
            val winReasons = leakingReasons.joinToString(" and ") // Conflict

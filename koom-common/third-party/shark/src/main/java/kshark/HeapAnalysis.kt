...
data class HeapAnalysisFailure(
    override val heapDumpFile: File, override val createdAtTimeMillis: Long, override val dumpDurationMillis: Long = DUMP_DURATION_UNKNOWN, override val analysisDurationMillis: Long,
    /**
...
===================================="""
    ...
    companion object {
        ...
        fun upgradeFrom20Deserialized(fromV20: HeapAnalysisSuccess): HeapAnalysisSuccess {
            val applicationLeaks = fromV20.applicationLeaks.map { it.leakTraceFromV20() }.groupBy { it.signature }.values.map {
                    ApplicationLeak(it)
                }
            
            val libraryLeaks = fromV20.libraryLeaks.map { it to it.leakTraceFromV20() }.groupBy { it.second.signature }.values.map { listOfPairs ->
                    val libraryLeakFrom20 = listOfPairs.first().first
                    LibraryLeak(pattern = libraryLeakFrom20.pattern, description = libraryLeakFrom20.description, leakTraces = listOfPairs.map { it.second })
                }

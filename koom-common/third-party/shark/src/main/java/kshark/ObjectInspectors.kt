...
enum class ObjectInspectors : ObjectInspector {
    ...
    KEYED_WEAK_REFERENCE {
        ...
        override val leakingObjectFilter = { heapObject: HeapObject ->
            KeyedWeakReferenceFinder.findKeyedWeakReferences(heapObject.graph).filter { it.hasReferent && it.isRetained }.any { reference ->
            ...
            if(heapObject is HeapInstance) {
                ...
                if(instanceClass.name.matches(ANONYMOUS_CLASS_NAME_PATTERN_REGEX)) {
                    ...
                    if(parentClassRecord.name == "java.lang.Object") {
                        try { // This is an anonymous class implementing an interface. The API does not give access
                    ...
                    } else { // Makes it easier to figure out which anonymous class we're looking at.
    ...
    companion object {
         ...
         */
        val jdkLeakingObjectFilters: List<LeakingObjectFilter> = createLeakingObjectFilters(EnumSet.allOf(ObjectInspectors::class.java))
        
        /**
         * Creates a list of [LeakingObjectFilter] based on the passed in [ObjectInspectors].
         */
        fun createLeakingObjectFilters(inspectors: Set<ObjectInspectors>): List<LeakingObjectFilter> = inspectors.mapNotNull { it.leakingObjectFilter }.map { filter ->

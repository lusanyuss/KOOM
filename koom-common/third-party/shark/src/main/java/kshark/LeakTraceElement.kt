...
import kshark.LeakNodeStatus.*
import kshark.LeakTrace.GcRootType
import kshark.LeakTraceElement.Holder.*
...
internal class LeakTraceElement : Serializable {
    ...
    fun gcRootTypeFromV20() = when (val gcRootLabel = labels!!.first {
    ...
    fun originObjectFromV20() = LeakTraceObject(
        ...
        }, className = className!!, labels = labels!!.filter { !it.startsWith("GC Root: ") }.toSet(), leakingStatus = when (leakStatus!!) {
            NOT_LEAKING -> LeakingStatus.NOT_LEAKING
            LEAKING -> LeakingStatus.LEAKING
            UNKNOWN -> LeakingStatus.UNKNOWN
        }, leakingStatusReason = leakStatusReason!!, retainedHeapByteSize = null, retainedObjectCount = null
    )

...
class HprofRecordReader internal constructor(
    header: HprofHeader, private val source: BufferedSource
) {
    ...
    init {
        val typeSizesMap = PrimitiveType.byteSizeByHprofType + (PrimitiveType.REFERENCE_HPROF_TYPE to identifierByteSize)
    ...
    fun readStringRecord(length: Long) = StringRecord(
        id = readId(), string = readUtf8(length - identifierByteSize)
    )
    
    fun readLoadClassRecord() = LoadClassRecord(
        classSerialNumber = readInt(), id = readId(), stackTraceSerialNumber = readInt(), classNameStringId = readId()
    )
    
    fun readStackFrameRecord() = StackFrameRecord(
        id = readId(), methodNameStringId = readId(), methodSignatureStringId = readId(), sourceFileNameStringId = readId(), classSerialNumber = readInt(), lineNumber = readInt()
    )
    
    fun readStackTraceRecord() = StackTraceRecord(
        stackTraceSerialNumber = readInt(), threadSerialNumber = readInt(), stackFrameIds = readIdArray(readInt())
    )
    
    fun readUnknownGcRootRecord() = Unknown(id = readId())
    
    fun readJniGlobalGcRootRecord() = JniGlobal(
        id = readId(), jniGlobalRefId = readId()
    )
    ...
    fun readNativeStackGcRootRecord() = NativeStack(
        id = readId(), threadSerialNumber = readInt()
    )
    ...
    fun readThreadObjectGcRootRecord() = ThreadObject(
        id = readId(), threadSerialNumber = readInt(), stackTraceSerialNumber = readInt()
    )
    ...
    fun readJniMonitorGcRootRecord() = JniMonitor(
        id = readId(), stackTraceSerialNumber = readInt(), stackDepth = readInt()
    )
    ...
    fun readInstanceDumpRecord(): InstanceDumpRecord {
        ...
        return InstanceDumpRecord(
            id = id, stackTraceSerialNumber = stackTraceSerialNumber, classId = classId, fieldValues = fieldValues
        )
    }
    ...
    fun readClassDumpRecord(): ClassDumpRecord {
        val id = readId() // stack trace serial number
        val stackTraceSerialNumber = readInt()
        val superclassId = readId() // class loader object ID
        val classLoaderId = readId() // signers object ID
        val signersId = readId() // protection domain object ID
        val protectionDomainId = readId() // reserved
        readId() // reserved
        readId()
        ...
        for (i in 0 until constantPoolCount) { // constant pool index
            skip(SHORT_SIZE)
        ...
        for (i in 0 until staticFieldCount) {
            ...
            staticFields.add(
                StaticFieldRecord(
                    nameStringId = nameStringId, type = type, value = value
                )
            )
        }
    ...
    fun readPrimitiveArrayDumpRecord(): PrimitiveArrayDumpRecord {
        val id = readId()
        val stackTraceSerialNumber = readInt() // length
    ...
    fun readObjectArrayDumpRecord(): ObjectArrayDumpRecord {
        ...
        return ObjectArrayDumpRecord(
            id = id, stackTraceSerialNumber = stackTraceSerialNumber, arrayClassId = arrayClassId, elementIds = elementIds
        )
    }
    ...
    fun skipClassDumpConstantPool() { // Skip over the constant pool
        val constantPoolCount = readUnsignedShort()
        for (i in 0 until constantPoolCount) { // constant pool index
            skip(SHORT.byteSize)
    ...
    fun readId(): Long { // As long as we don't interpret IDs, reading signed values here is fine.

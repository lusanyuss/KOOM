...
sealed class HeapObject {
    ...
    class HeapClass internal constructor(
        private val hprofGraph: HprofHeapGraph, private val indexedObject: IndexedClass, override val objectId: Long, override val objectIndex: Int
    ) : HeapObject() {
        ...
        val classHierarchy: Sequence<HeapClass>
            get() {
                if(_classHierarchy == null) _classHierarchy = generateSequence(this) { it.superclass }
        ...
        fun readStaticFields(): Sequence<HeapField> {
            return readRecordStaticFields().asSequence().map { fieldRecord ->
                    HeapField(
                        this, hprofGraph.staticFieldName(objectId, fieldRecord), HeapValue(hprofGraph, fieldRecord.value)
                    )
                }
        }
    ...
    class HeapInstance internal constructor(
        private val hprofGraph: HprofHeapGraph, internal val indexedObject: IndexedInstance, override val objectId: Long, override val objectIndex: Int
    ) : HeapObject() {
         ...
         */
        infix fun instanceOf(className: String): Boolean = instanceClass.classHierarchy.any { it.name == className }
        
        /**
         * Returns true if this is an instance of [expectedClass] or an instance of a subclass of that
         * class.
         */
        infix fun instanceOf(expectedClass: KClass<*>) = this instanceOf expectedClass.java.name
        
        /**
         * Returns true if this is an instance of [expectedClass] or an instance of a subclass of that
         * class.
         */
        infix fun instanceOf(expectedClass: HeapClass) = instanceClass.classHierarchy.any { it.objectId == expectedClass.objectId }
        
        /**
         * @see readField
         */
        fun readField(
            declaringClass: KClass<out Any>, fieldName: String
        ): HeapField? {
        ...
        fun readFields(): Sequence<HeapField> {
            ...
            return instanceClass.classHierarchy.map { heapClass ->
                    heapClass.readRecordFields().asSequence().map { fieldRecord ->
        ...
        fun readAsJavaString(): String? {
            ...
            when (val valueRecord = this["java.lang.String", "value"]!!.value.asObject!!.readRecord()) {
                is CharArrayDump -> { // < API 23
                    // As of Marshmallow, substrings no longer share their parent strings' char arrays
                    // eliminating the need for String.offset
                    // https://android-review.googlesource.com/#/c/83611/
                    val offset = this["java.lang.String", "offset"]?.value?.asInt
                    
                    val chars = if(count != null && offset != null) { // Handle heap dumps where all primitive arrays have been replaced with empty arrays,
                ...
                is ByteArrayDump -> {
                    ...
                    "'value' field ${this["java.lang.String", "value"]!!.value} was expected to be either" + " a char or byte array in string instance with id $objectId"
                )
            }
        }
    ...
    class HeapObjectArray internal constructor(
        private val hprofGraph: HprofHeapGraph, internal val indexedObject: IndexedObjectArray, override val objectId: Long, override val objectIndex: Int
    ) : HeapObject() {
        ...
        fun readElements(): Sequence<HeapValue> {
            return readRecord().elementIds.asSequence().map { HeapValue(hprofGraph, ReferenceHolder(it)) }
        }
    ...
    class HeapPrimitiveArray internal constructor(
        private val hprofGraph: HprofHeapGraph, private val indexedObject: IndexedPrimitiveArray, override val objectId: Long, override val objectIndex: Int
    ) : HeapObject() {
    ...
    companion object {
        ...
        internal val primitiveTypesByPrimitiveArrayClassName = PrimitiveType.values().map { "${it.name.toLowerCase(Locale.US)}[]" to it }.toMap()
        
        private val primitiveWrapperClassNames = setOf<String>(
            Boolean::class.javaObjectType.name,
            Char::class.javaObjectType.name,

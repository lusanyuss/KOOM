...
class HprofHeapGraph internal constructor(
    private val header: HprofHeader, private val reader: RandomAccessHprofReader, private val index: HprofInMemoryIndex
) : CloseableHeapGraph {
    ...
    override val objects: Sequence<HeapObject>
        get() {
            var objectIndex = 0
            return index.indexedObjectSequence().map {
    ...
    override val classes: Sequence<HeapClass>
        get() {
            var objectIndex = 0
            return index.indexedClassSequence().map {
    ...
    override val instances: Sequence<HeapInstance>
        get() {
            ...
            return index.indexedInstanceSequence().map {
                    val objectId = it.first
                    val indexedObject = it.second
    ...
    internal fun fieldName(
        classId: Long, fieldRecord: FieldRecord
    ): String {
        return index.fieldName(classId, fieldRecord.nameStringId)
    }
    
    internal fun staticFieldName(
        classId: Long, fieldRecord: StaticFieldRecord
    ): String {
        return index.fieldName(classId, fieldRecord.nameStringId)
    }
    
    internal fun createFieldValuesReader(record: InstanceDumpRecord) = FieldValuesReader(record, identifierByteSize)
    ...
    internal fun readObjectArrayDumpRecord(
        objectId: Long, indexedObject: IndexedObjectArray
    ...
    internal fun readPrimitiveArrayDumpRecord(
        objectId: Long, indexedObject: IndexedPrimitiveArray
    ...
    internal fun readClassDumpRecord(
        objectId: Long, indexedObject: IndexedClass
    ): ClassDumpRecord {
    ...
    internal fun readInstanceDumpRecord(
        objectId: Long, indexedObject: IndexedInstance
    ): InstanceDumpRecord {
        return readObjectRecord(objectId, indexedObject) {
    ...
    private fun <T : ObjectRecord> readObjectRecord(
        objectId: Long, indexedObject: IndexedObject, readBlock: HprofRecordReader.() -> T
    ): T {
        val objectRecordOrNull = objectCache[objectId]
        @Suppress("UNCHECKED_CAST") if(objectRecordOrNull != null) {
    ...
    private fun wrapIndexedObject(
        objectIndex: Int, indexedObject: IndexedObject, objectId: Long
    ): HeapObject {
    ...
    companion object {
        ...
        fun File.openHeapGraph(
            proguardMapping: ProguardMapping? = null, indexedGcRootTypes: Set<HprofRecordTag> = HprofIndex.defaultIndexedGcRootTags()
        ): CloseableHeapGraph {
        ...
        @Deprecated(
            "Replaced by HprofIndex.indexRecordsOf().openHeapGraph() or File.openHeapGraph()", replaceWith = ReplaceWith(
                "HprofIndex.indexRecordsOf(hprof, proguardMapping, indexedGcRootTypes)" + ".openHeapGraph()"
            )
        )
        fun indexHprof(
            hprof: Hprof, proguardMapping: ProguardMapping? = null, indexedGcRootTypes: Set<KClass<out GcRoot>> = deprecatedDefaultIndexedGcRootTypes()
        ): HeapGraph {
        ...
        private fun deprecatedDefaultIndexedGcRootTypes() = setOf(
            JniGlobal::class, JavaFrame::class, JniLocal::class, MonitorUsed::class, NativeStack::class, StickyClass::class, ThreadBlock::class, ThreadObject::class, JniMonitor::class
        )
    }
}

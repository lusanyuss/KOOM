...
import kshark.HprofWriter.Companion.openWriterFor
...
class HprofWriter private constructor(
    private val sink: BufferedSink, val hprofHeader: HprofHeader
) : Closeable {
    
    @Deprecated(
        "Replaced by HprofWriter.hprofHeader.identifierByteSize", ReplaceWith("hprofHeader.identifierByteSize")
    )
    val identifierByteSize: Int
        get() = hprofHeader.identifierByteSize
    
    @Deprecated(
        "Replaced by HprofWriter.hprofHeader.version", ReplaceWith("hprofHeader.version")
    )
        ...
        when (record) {
            ...
            is ClassDumpRecord -> {
                with(workBuffer) {
                    ...
                    writeId(record.protectionDomainId) // reserved
                    writeId(0) // reserved
                    writeId(0)
                    writeInt(record.instanceSize) // Not writing anything in the constant pool
    ...
    private fun BufferedSink.writeNonHeapRecord(
        tag: Int, block: BufferedSink.() -> Unit
    ) {
        flushHeapBuffer()
        workBuffer.block()
    ...
    private fun BufferedSink.writeTagHeader(
        ...
        writeByte(tag) // number of microseconds since the time stamp in the header
        writeInt(0)
    ...
    companion object {
        ...
        fun openWriterFor(
            hprofFile: File, hprofHeader: HprofHeader = HprofHeader()
        ): HprofWriter {
            return openWriterFor(Okio.buffer(Okio.sink(hprofFile.outputStream())), hprofHeader)
        }
        
        fun openWriterFor(
            hprofSink: BufferedSink, hprofHeader: HprofHeader = HprofHeader()
        ): HprofWriter {
        ...
        @Deprecated(
            "Replaced by HprofWriter.openWriterFor()", ReplaceWith(
                "kshark.HprofWriter.openWriterFor(hprofFile)"
            )
        )
        fun open(
            hprofFile: File, identifierByteSize: Int = 4, hprofVersion: Hprof.HprofVersion = Hprof.HprofVersion.ANDROID
        ): HprofWriter = openWriterFor(
            hprofFile, HprofHeader(
                version = HprofVersion.valueOf(hprofVersion.name), identifierByteSize = identifierByteSize
            )
        )
    }
}

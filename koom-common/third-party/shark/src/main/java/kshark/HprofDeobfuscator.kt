...
class HprofDeobfuscator {
     ...
     */
    fun deobfuscate(
        proguardMapping: ProguardMapping, inputHprofFile: File,
        /**
         ...
         */
        outputHprofFile: File = File(inputHprofFile.parent, inputHprofFile.name.replace(
        ...
        return writeHprofRecords(
            inputHprofFile, outputHprofFile, proguardMapping, hprofStringCache, classNames, maxId + 1
        )
    }
    ...
    private fun readHprofRecords(
        ...
        reader.readRecords(setOf(HprofRecord::class), OnHprofRecordListener { _, record ->
            when (record) {
                is StringRecord -> {
    ...
    private fun writeHprofRecords(
        inputHprofFile: File, outputHprofFile: File, proguardMapping: ProguardMapping, hprofStringCache: Map<Long, String>, classNames: Map<Long, Long>, firstId: Long
    ): File {
        var id = firstId
        
        val hprofHeader = parseHeaderOf(inputHprofFile)
        val reader = StreamingHprofReader.readerFor(inputHprofFile, hprofHeader).asStreamingRecordReader()
        HprofWriter.openWriterFor(
            outputHprofFile, hprofHeader = HprofHeader(
                identifierByteSize = hprofHeader.identifierByteSize, version = hprofHeader.version
            )
        ).use { writer ->
            reader.readRecords(setOf(HprofRecord::class), OnHprofRecordListener { _, record -> // HprofWriter automatically emits HeapDumpEndRecord, because it flushes
    ...
    private fun createDeobfuscatedStringRecord(
        record: StringRecord, proguardMapping: ProguardMapping, hprofStringCache: Map<Long, String>
    ): StringRecord {
    ...
    private fun createDeobfuscatedClassDumpRecord(
        record: ClassDumpRecord, proguardMapping: ProguardMapping, hprofStringCache: Map<Long, String>, classNames: Map<Long, Long>, maxId: Long
        ...
        val newFields = record.fields.map { field ->
            ...
            val deobfuscatedName = proguardMapping.deobfuscateFieldName(className, fieldName)
        ...
        recordsToWrite.add(
            ClassDumpRecord(
                record.id, record.stackTraceSerialNumber, record.superclassId, record.classLoaderId, record.signersId, record.protectionDomainId, record.instanceSize, newStaticFields, newFields
            )
        )

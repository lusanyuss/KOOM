...
internal class PathFinder(
    private val graph: HeapGraph, private val listener: OnAnalysisProgressListener, referenceMatchers: List<ReferenceMatcher>
) {
    
    class PathFindingResults(
        val pathsToLeakingObjects: List<ReferencePathNode>, val dominatorTree: DominatorTree?
    )
    
    sealed class VisitTracker {
        
        abstract fun visited(
            objectId: Long, parentObjectId: Long
        ): Boolean
    ...
    private class State(
        val leakingObjectIds: LongScatterSet, val sizeOfObjectInstances: Int, val computeRetainedHeapSize: Boolean, val javaLangObjectId: Long, estimatedVisitedObjects: Int
    ) {
    ...
    fun findPathsFromGcRoots(
        leakingObjectIds: Set<Long>, computeRetainedHeapSize: Boolean
    ): PathFindingResults {
    ...
    private fun determineSizeOfObjectInstances(
        objectClass: HeapClass?, graph: HeapGraph
    ): Int {
        return if(objectClass != null) { // In Android 16 ClassDumpRecord.instanceSize for java.lang.Object can be 8 yet there are 0
    ...
    private fun State.findPathsFromGcRoots(): PathFindingResults {
        ...
        return PathFindingResults(
            shortestPathsToLeakingObjects, if(visitTracker is Dominated) visitTracker.dominatorTree else null
        )
    }
        ...
        gcRoots.forEach { (objectRecord, gcRoot) ->
            when (gcRoot) {
                ...
                is JavaFrame -> {
                    ...
                    if(threadPair == null) { // Could not find the thread that this java frame is for.
                        enqueue(NormalRootNode(gcRoot.id, gcRoot))
                    } else {
                        ...
                        if(referenceMatcher !is IgnoredReferenceMatcher) {
                            ...
                            val refFromParentType = LOCAL // Unfortunately Android heap dumps do not include stack trace data, so
                            ...
                            val childNode = if(referenceMatcher is LibraryLeakReferenceMatcher) {
                                LibraryLeakChildNode(
                                    objectId = gcRoot.id, parent = rootNode, refFromParentType = refFromParentType, refFromParentName = refFromParentName, matcher = referenceMatcher
                                )
                            } else {
                                NormalNode(
                                    objectId = gcRoot.id, parent = rootNode, refFromParentType = refFromParentType, refFromParentName = refFromParentName
                                )
                            }
                            enqueue(childNode)
                        }
    ...
    private fun sortedGcRoots(): List<Pair<HeapObject, GcRoot>> {
        ...
        return graph.gcRoots.filter { gcRoot -> // GC roots sometimes reference objects that don't exist in the heap dump
                // See https://github.com/square/leakcanary/issues/1516
                graph.objectExists(gcRoot.id)
            }.map { graph.findObjectById(it.id) to it }
            .sortedWith(Comparator { (graphObject1, root1), (graphObject2, root2) -> // Sorting based on pattern name first. In reverse order so that ThreadObject is before JavaLocalPattern
    ...
    private fun State.visitClassRecord(
        heapClass: HeapClass, parent: ReferencePathNode
    ) {
        ...
        for (staticField in heapClass.readStaticFields()) {
            ...
            val node = when (val referenceMatcher = ignoredStaticFields[fieldName]) {
                null -> NormalNode(
                    objectId = objectId, parent = parent, refFromParentType = STATIC_FIELD, refFromParentName = fieldName
                )
                is LibraryLeakReferenceMatcher -> LibraryLeakChildNode(
                    objectId = objectId, parent = parent, refFromParentType = STATIC_FIELD, refFromParentName = fieldName, matcher = referenceMatcher
                )
    ...
    private fun State.visitInstance(
        instance: HeapInstance, parent: ReferencePathNode
    ) {
        ...
        val classHierarchy = instance.instanceClass.classHierarchyWithoutJavaLangObject(javaLangObjectId)
        val fieldNamesAndValues = instance.readAllNonNullFieldsOfReferenceType(classHierarchy)
    ...
    private class InstanceRefField(
        val declaringClassId: Long, val refObjectId: Long, val fieldName: String
    )
    
    private fun HeapInstance.readAllNonNullFieldsOfReferenceType(
        classHierarchy: List<HeapClass>
    ): MutableList<InstanceRefField> { // Assigning to local variable to avoid repeated lookup and cast:
        ...
        for (heapClass in classHierarchy) {
            for (fieldRecord in heapClass.readRecordFields()) {
                if(fieldRecord.type != PrimitiveType.REFERENCE_HPROF_TYPE) { // Skip all fields that are not references. Track how many bytes to skip
                    skipBytesCount += hprofGraph.getRecordSize(fieldRecord)
                } else { // Initialize id reader if it's not yet initialized. Replaces `lazy` without synchronization
                    if(fieldReader == null) {
    ...
    private fun HeapGraph.getRecordSize(field: FieldRecord) = when (field.type) {
    ...
    private fun State.visitObjectArray(
        objectArray: HeapObjectArray, parent: ReferencePathNode
    ) {
        ...
        nonNullElementIds.forEachIndexed { index, elementId ->
            ...
            enqueue(
                NormalNode(
                    objectId = elementId, parent = parent, refFromParentType = ARRAY_ENTRY, refFromParentName = name
                )
            )
        }
    }
    ...
    private fun isOverThresholdInstance(graphObject: HeapInstance): Boolean {
        if(graphObject.instanceClassName.startsWith("java.util") || graphObject.instanceClassName.startsWith("android.util") || graphObject.instanceClassName.startsWith("java.lang.String") //|| graphObject.instanceClassName.startsWith("kotlin.collections")
    ...
    private fun State.enqueue(
        ...
        val visitLast = visitingLast || node is LibraryLeakNode || // We deprioritize thread objects because on Lollipop the thread local values are stored
                // as a field.
                (node is RootNode && node.gcRoot is ThreadObject) || (node is NormalNode && node.parent is RootNode && node.parent.gcRoot is JavaFrame)
        ...
        if(alreadyEnqueued) { // Has already been enqueued and would be added to visit last => don't enqueue.
            if(visitLast) {
                return
        ...
        if(alreadyEnqueued) { // Move from "visit last" to "visit first" queue.
        ...
        if(!isLeakingObject) {
            val skip = when (val graphObject = graph.findObjectById(node.objectId)) {
                ...
                is HeapInstance -> when {
                    ...
                    graphObject.instanceClassName == "java.lang.String" -> { // We ignore the fact that String references a value array to avoid having
                ...
                is HeapObjectArray -> when {
                    graphObject.isSkippablePrimitiveWrapperArray -> { // Same optimization as we did for String above, as we know primitive wrapper arrays
                        // aren't interesting.
                        true
                    }
                    else -> false
...
private val skippablePrimitiveWrapperArrayTypes = setOf(
    Boolean::class, Char::class, Float::class, Double::class, Byte::class, Short::class, Int::class, Long::class

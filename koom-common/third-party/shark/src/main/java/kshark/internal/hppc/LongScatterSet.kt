...
internal class LongScatterSet(expectedElements: Int = 4) {
    ...
    private fun shiftConflictingKeys(inputGapSlot: Int) {
        ...
        val keys = keys
        val mask = mask // Perform shifts of conflicting keys to fill in the gap.
        var distance = 0
        while (true) {
            ...
            if(shift >= distance) { // Entry at this position was originally at or before the gap slot.
    ...
    private fun rehash(fromKeys: LongArray) { // Rehash all stored keys into the new buffers.
        val keys = this.keys
        val mask = this.mask
        var existing: Long
     ...
     */
    private fun allocateBuffers(arraySize: Int) { // Ensure no change is done if we hit an OOM.
        val prevKeys = this.keys
        try {
            ...
            throw RuntimeException(
                String.format(
                    Locale.ROOT, "Not enough memory to allocate buffers for rehashing: %,d -> %,d", size(), arraySize
                ), e
            )
        }

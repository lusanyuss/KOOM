...
internal class LongObjectScatterMap<T> {
     ...
     */
    private fun rehash(
        fromKeys: LongArray, fromValues: Array<T?>
    ...
    private fun allocateBuffers(arraySize: Int) {
        ...
        try {
            ...
            @Suppress("UNCHECKED_CAST") this.values = arrayOfNulls<Any?>(arraySize + emptyElementSlot) as Array<T?>
        } catch (e: OutOfMemoryError) {
            this.keys = prevKeys
            this.values = prevValues
            throw RuntimeException(
                String.format(
                    Locale.ROOT, "Not enough memory to allocate buffers for rehashing: %,d -> %,d", this.mask + 1, arraySize
                ), e
            )
        }
    ...
    private fun allocateThenInsertThenRehash(
        slot: Int, pendingKey: Long, pendingValue: T
    ) {

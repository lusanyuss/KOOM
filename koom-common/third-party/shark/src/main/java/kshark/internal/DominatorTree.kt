...
import kshark.internal.ObjectDominators.DominatorNode
import kshark.internal.hppc.LongLongScatterMap
import kshark.internal.hppc.LongLongScatterMap.ForEachCallback
...
internal class DominatorTree(expectedElements: Int = 4) {
    ...
    fun updateDominated(
        ...
        if(!hasDominator || parentObjectId == ValueHolder.NULL_REFERENCE) {
            ...
            if(currentDominator != ValueHolder.NULL_REFERENCE) { // We're looking for the Lowest Common Dominator between currentDominator and
    ...
    fun computeRetainedSizes(
        retainedObjectIds: Set<Long>, computeSize: (Long) -> Int
        ...
        dominated.forEach(object : ForEachCallback {
            override fun onEntry(
                ...
                if(value != ValueHolder.NULL_REFERENCE) {
                    ...
                    while (dominator != ValueHolder.NULL_REFERENCE) { // If dominator is a node
                        if(nodeRetainedSizes.containsKey(dominator)) { // Update dominator for all objects in the dominator path so far to directly point

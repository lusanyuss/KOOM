...
private constructor(
    ...
    init { // Allocate temp storage (which may be increased later if necessary)
        val len = a.size / entrySize
        val newArray = ByteArray(
            entrySize * if(len < 2 * INITIAL_TMP_STORAGE_LENGTH) len.ushr(1)
     ...
     */
    private fun mergeLo(
        base1: Int, len1: Int, base2: Int, len2: Int
    ) {
        var len1 = len1
        var len2 = len2
        if(DEBUG) assert(len1 > 0 && len2 > 0 && base1 + len1 == base2) // Copy first run into temp array
        ...
        var dest = base1      // Indexes int a // Move first element of second run and deal with degenerate cases
    ...
    private fun mergeHi(
        ...
        var len1 = len1
        var len2 = len2
        if(DEBUG) assert(len1 > 0 && len2 > 0 && base1 + len1 == base2) // Copy second run into temp array
        val a = this.a // For performance
        ...
        var dest = base2 + len2 - 1     // Indexes into a // Move last element of first run and deal with degenerate cases
        var destIndex = dest * entrySize
        val cursor1Index = cursor1 * entrySize
        for (i in 0 until entrySize) {
    ...
    private fun ensureCapacity(minCapacity: Int): ByteArray {
        if(tmp!!.size < minCapacity * entrySize) { // Compute smallest power of 2 > minCapacity
            ...
            if(newSize < 0) // Not bloody likely!
                newSize = minCapacity
            else newSize = Math.min(newSize, (a.size / entrySize).ushr(1))
    ...
    companion object {
         ...
         */
        fun sort(
            a: ByteArray, entrySize: Int, c: ByteArrayComparator
        ) {
            sort(a, 0, a.size / entrySize, entrySize, c)
        }
        
        fun sort(
            a: ByteArray, lo: Int, hi: Int, entrySize: Int, c: ByteArrayComparator
        ) {
            var lo = lo
            ...
            do { // Identify next run
                var runLen = countRunAndMakeAscending(a, lo, hi, entrySize, c) // If run is short, extend to min(minRun, nRemaining)
                if(runLen < minRun) {
        ...
        private fun checkStartAndEnd(
            len: Int, start: Int, end: Int
        ) {
            if(start < 0 || end > len) {
                throw ArrayIndexOutOfBoundsException(
                    "start < 0 || end > len." + " start=" + start + ", end=" + end + ", len=" + len
                )
            }
            if(start > end) {
        ...
        private fun binarySort(
            a: ByteArray, lo: Int, hi: Int, start: Int, entrySize: Int, c: ByteArrayComparator
        ) {
            var start = start
             ...
             */
                while (left < right) {
                    ...
                    if(c.compare(entrySize, pivot, 0, a, mid) < 0) right = mid
                    else left = mid + 1
                }
                if(DEBUG) assert(left == right)/*
             ...
             */
                val n = start - left  // The number of elements to move // Switch is just an optimization for arraycopy in default case
                when (n) {
                    2 -> {
        ...
        private fun countRunAndMakeAscending(
            ...
            if(comparison < 0) { // Descending
                while (runHi < hi && c.compare(entrySize, a, runHi, a, runHi - 1) < 0) runHi++
        ...
        private fun reverseRange(
            a: ByteArray, lo: Int, hi: Int, entrySize: Int
        ) {
            var lo = lo
            var hi = hi
            hi--
        ...
        private fun gallopLeft(
            keyArray: ByteArray, // Index already divided by entrySize
            keyIndex: Int, a: ByteArray, base: Int, len: Int, hint: Int, entrySize: Int, c: ByteArrayComparator
        ): Int {
            if(DEBUG) assert(len > 0 && hint >= 0 && hint < len)
            var lastOfs = 0
            var ofs = 1
            if(c.compare(entrySize, keyArray, keyIndex, a, base + hint) > 0) { // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
                val maxOfs = len - hint
                ...
                if(ofs > maxOfs) ofs = maxOfs // Make offsets relative to base
                lastOfs += hint
                ofs += hint
         ...
         */
            ...
            while (lastOfs < ofs) {
                ...
                if(c.compare(entrySize, keyArray, keyIndex, a, base + m) > 0) lastOfs = m + 1  // a[base + m] < key
        ...
        private fun gallopRight(
            ...
            var ofs = 1
            var lastOfs = 0
            if(c.compare(entrySize, keyArray, keyIndex, a, base + hint) < 0) { // Gallop left until a[b+hint - ofs] <= key < a[b+hint - lastOfs]
                val maxOfs = hint + 1
                while (ofs < maxOfs && c.compare(entrySize, keyArray, keyIndex, a, base + hint - ofs) < 0) {
                    lastOfs = ofs
                    ofs = ofs * 2 + 1
                ...
                if(ofs > maxOfs) ofs = maxOfs // Make offsets relative to b
                val tmp = lastOfs
                lastOfs = hint - ofs
                ofs = hint - tmp
            } else { // a[b + hint] <= key
         ...
         */
            ...
            while (lastOfs < ofs) {
                val m = lastOfs + (ofs - lastOfs).ushr(1)
                if(c.compare(entrySize, keyArray, keyIndex, a, base + m) < 0) ofs = m          // key < a[b + m]

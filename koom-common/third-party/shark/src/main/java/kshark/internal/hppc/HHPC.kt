...
internal object HHPC {
    ...
    fun minBufferSize(
        elements: Int, loadFactor: Double
    ): Int {
        var length = Math.ceil(elements / loadFactor).toLong()
        ...
        if(length > MAX_HASH_ARRAY_LENGTH) {
            throw RuntimeException(
                String.format(
                    Locale.ROOT, "Maximum array size exceeded for this load factor (elements: %d, load factor: %f)", elements, loadFactor
                )
            )
        }
    ...
    fun expandAtCount(
        arraySize: Int, loadFactor: Double
    ): Int {
        return Math.min(arraySize - 1, Math.ceil(arraySize * loadFactor).toInt())
    }
    
    fun nextBufferSize(
        arraySize: Int, elements: Int, loadFactor: Double
    ): Int {

...
class ObjectReporter constructor(val heapObject: HeapObject) {
     ...
     */
    @Deprecated(
        "Replace likelyLeakingReasons with leakingReasons", replaceWith = ReplaceWith(
            "leakingReasons"
        )
    )
     ...
     */
    fun whenInstanceOf(
        expectedClass: KClass<out Any>, block: ObjectReporter.(HeapInstance) -> Unit
    ) {
     ...
     */
    fun whenInstanceOf(
        expectedClassName: String, block: ObjectReporter.(HeapInstance) -> Unit
    ) {

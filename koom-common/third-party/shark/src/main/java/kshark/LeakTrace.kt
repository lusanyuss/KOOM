...
import kshark.LeakTrace.GcRootType.JAVA_FRAME
...
data class LeakTrace(
     ...
     */
    val gcRootType: GcRootType, val referencePath: List<LeakTraceReference>, val leakingObject: LeakTraceObject
) : Serializable {
    ...
    val retainedHeapByteSize: Int?
        get() {
            ...
            return allObjects.filter { it.leakingStatus == LEAKING }.mapNotNull { it.retainedHeapByteSize } // The minimum released is the max held by a leaking object.
                .max()
        }
    ...
    val retainedObjectCount: Int?
        get() {
            val allObjects = listOf(leakingObject) + referencePath.map { it.originObject }
            return allObjects.filter { it.leakingStatus == LEAKING }.mapNotNull { it.retainedObjectCount } // The minimum released is the max held by a leaking object.
                .max()
        }
    ...
    val suspectReferenceSubpath
        get() = referencePath.asSequence().filterIndexed { index, _ ->
    ...
    val signature: String
        get() = suspectReferenceSubpath.joinToString(separator = "") { element ->
      ...
      """.trimIndent()
        ...
        referencePath.forEachIndexed { index, element ->
            ...
            result += originObject.toString(
                firstLinePrefix = "├─ ", additionalLinesPrefix = "│    ", showLeakingStatus = showLeakingStatus,
                /**
        ...
        result += leakingObject.toString(
            firstLinePrefix = "╰→ ", additionalLinesPrefix = "$ZERO_WIDTH_SPACE     ", showLeakingStatus = showLeakingStatus
        )
        return result
    }
    
    enum class GcRootType(val description: String) {
        JNI_GLOBAL("Global variable in native code"), JNI_LOCAL("Local variable in native code"), JAVA_FRAME("Java local variable"), NATIVE_STACK("Input or output parameters in native code"), STICKY_CLASS(
            "System class"
        ),
        THREAD_BLOCK("Thread block"), MONITOR_USED(
            "Monitor (anything that called the wait() or notify() methods, or that is synchronized.)"
        ),
        THREAD_OBJECT("Thread object"), JNI_MONITOR("Root JNI monitor"), ;
    ...
    internal fun fromV20(retainedHeapByteSize: Int?) = LeakTrace(gcRootType = elements!!.first().gcRootTypeFromV20(), referencePath = when {
        elements.isEmpty() -> emptyList()
        else -> elements.subList(0, elements.lastIndex - 1).map { it.referencePathElementFromV20() }
    }, leakingObject = elements.last().originObjectFromV20())
    
    companion object {
        private fun getNextElementString(
            leakTrace: LeakTrace, reference: LeakTraceReference, index: Int, showLeakingStatus: Boolean
        ): String {
            val static = if(reference.referenceType == STATIC_FIELD) " static" else ""
            val referenceLine = "    ↓$static ${reference.owningClassSimpleName}.${reference.referenceDisplayName}"

...
class AndroidResourceIdNames private constructor(
    private val resourceIds: IntArray, private val names: Array<String>
) {
    ...
    companion object {
        ...
        @Synchronized
        fun saveToMemory(
            getResourceTypeName: (Int) -> String?, getResourceEntryName: (Int) -> String?
        ) {
            if(holderField != null) {
                return
            }
            ...
            val resourceIds = idToNamePairs.map { it.first }.toIntArray()
            val names = idToNamePairs.map { it.second }.toTypedArray()
            ...
            return graph.context.getOrPut(AndroidResourceIdNames::class.java.name) {
                ...
                holderClass?.let {
                    ...
                    holderField.valueAsInstance?.let { instance ->
                        ...
                        val resourceIds = (resourceIdsArray.readRecord() as IntArrayDump).array
                        val names = instance[className, "names"]!!.valueAsObjectArray!!.readElements().map { it.readAsJavaString()!! }.toList().toTypedArray()

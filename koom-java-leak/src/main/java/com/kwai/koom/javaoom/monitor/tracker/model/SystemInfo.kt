...
internal object SystemInfo {
    ...
    fun refresh() {
        ...
        File("/proc/self/status").forEachLineQuietly { line ->
            if(procStatus.vssInKb != 0 && procStatus.rssInKb != 0 && procStatus.thread != 0) return@forEachLineQuietly
    ...
    data class MemInfo(
        var totalInKb: Int = 0, var freeInKb: Int = 0, var availableInKb: Int = 0, var IONHeap: Int = 0, var cmaTotal: Int = 0, var rate: Float = 0f
    )
    
    data class JavaHeap(
        var max: Long = 0, var total: Long = 0, var free: Long = 0, var used: Long = 0, var rate: Float = 0f
    )
    ...
    private fun File.forEachLineQuietly(charset: Charset = Charsets.UTF_8, action: (line: String) -> Unit) {
        kotlin.runCatching { // Note: close is called at forEachLineQuietly

...
object OOMMonitor : LoopMonitor<OOMMonitorConfig>(), LifecycleEventObserver {
    ...
    private val mOOMTrackers = mutableListOf(
        HeapOOMTracker(), ThreadOOMTracker(), FdOOMTracker(), PhysicalMemoryOOMTracker(), FastHugeMemoryOOMTracker()
    )
    ...
    override fun call(): LoopState {
        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP || Build.VERSION.SDK_INT > Build.VERSION_CODES.S) {
    ...
    private fun isExceedAnalysisTimes(): Boolean {
        MonitorLog.i(
            TAG, "OOMPreferenceManager.getAnalysisTimes:${OOMPreferenceManager.getAnalysisTimes()}"
        )
        ...
        return (OOMPreferenceManager.getAnalysisTimes() > monitorConfig.analysisMaxTimesPerVersion).also { if(it) MonitorLog.e(TAG, "current version is out of max analysis times!") }
    }
    
    private fun isExceedAnalysisPeriod(): Boolean {
        MonitorLog.i(
            TAG, "OOMPreferenceManager.getFirstAnalysisTime():" + OOMPreferenceManager.getFirstLaunchTime()
        )
        ...
        return (analysisPeriod > monitorConfig.analysisPeriodPerVersion).also { if(it) MonitorLog.e(TAG, "current version is out of max analysis period!") }
    }
        ...
        for (file in hprofAnalysisDir.listFiles().orEmpty()) {
            ...
            if(file.canonicalPath.endsWith(".hprof")) {
                ...
                if(!jsonFile.exists()) {
                    ...
                    MonitorLog.i(
                        TAG, if(jsonFile.length() == 0L) "last analysis isn't succeed, delete file"
    ...
    private fun startAnalysisService(
        hprofFile: File, jsonFile: File, reason: String
    ) {
        ...
        HeapAnalysisService.startAnalysisService(getApplication(), hprofFile.canonicalPath, jsonFile.canonicalPath, extraData, object : AnalysisReceiver.ResultCallBack {

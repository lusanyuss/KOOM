...
class ThreadOOMTracker : OOMTracker() {
    ...
    override fun track(): Boolean {
        ...
        if(threadCount > monitorConfig.threadThreshold && threadCount >= mLastThreadCount - THREAD_COUNT_THRESHOLD_GAP) {
            ...
            MonitorLog.i(
                TAG, "[meet condition] " + "overThresholdCount:$mOverThresholdCount" + ", threadCount: $threadCount"
            )
    ...
    private fun dumpThreadIfNeed() {
        ...
        val threadNames = runCatching { File("/proc/self/task").listFiles() }.getOrElse {
        ...
        OOMFileManager.createDumpFile(OOMFileManager.threadDumpDir).run {

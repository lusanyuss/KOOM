...
internal object OOMPreferenceManager {
    ...
    fun increaseAnalysisTimes() {
        mPreferences.edit().also { clearUnusedPreference(mPreferences, it) }.putInt("${mPrefix}times", mPreferences.getInt("${mPrefix}times", 0) + 1).apply()
    }
    ...
    fun setFirstLaunchTime(time: Long) {
        ...
        return mPreferences.edit().putLong("${mPrefix}first_analysis_time", time).apply()
    }

...
class HeapAnalysisService : IntentService("HeapAnalysisService") {
    companion object {
        ...
        fun startAnalysisService(
            context: Context, hprofFile: String?, jsonFile: String?, extraData: AnalysisExtraData, resultCallBack: AnalysisReceiver.ResultCallBack?
        ) {
    ...
    private fun buildJson(intent: Intent?) {
        mLeakModel.runningInfo = HeapReport.RunningInfo().apply {
            ...
            MonitorLog.i(
                TAG, "handle Intent, fdCount:${fdCount} pss:${pss} rss:${rss} vss:${vss} " + "threadCount:${threadCount}"
            )
    ...
    private fun filterLeakingObjects() {
        ...
        val fragmentHeapClass =
            mHeapGraph.findClassByName(ANDROIDX_FRAGMENT_CLASS_NAME) ?: mHeapGraph.findClassByName(NATIVE_FRAGMENT_CLASS_NAME) ?: mHeapGraph.findClassByName(SUPPORT_FRAGMENT_CLASS_NAME)
        ...
        val classHierarchyMap = mutableMapOf<Long, Pair<Long, Long>>() //记录class objects数量
        ...
        for (instance in mHeapGraph.instances) {
            ...
            if(activityHeapClass?.objectId == superId4) {
                ...
                if(destroyField.value.asBoolean!! || finishedField.value.asBoolean!!) {
                    ...
                    MonitorLog.i(
                        TAG,
                        "activity name : " + instance.instanceClassName + " mDestroyed:" + destroyField.value.asBoolean + " mFinished:" + finishedField.value.asBoolean + " objectId:" + (instance.objectId and 0xffffffffL)
                    )
                    if(objectCounter.leakCnt <= SAME_CLASS_LEAK_OBJECT_PATH_THRESHOLD) {
                        ...
                        MonitorLog.i(
                            OOM_ANALYSIS_TAG, instance.instanceClassName + " objectId:" + instance.objectId
                        )
                    }
                }
                continue
            ...
            if(fragmentHeapClass?.objectId == superId1) {
                ...
                if(fragmentManager != null && fragmentManager.value.asObject == null) {
                    val mCalledField = instance[fragmentHeapClass.name, FRAGMENT_MCALLED_FIELD_NAME] //mCalled为true且fragment manager为空时认为fragment已经destroy
            ...
            if(bitmapHeapClass?.objectId == superId1) {
                ...
                if(width * height >= DEFAULT_BIG_BITMAP) {
                    val objectCounter = updateClassObjectCounterMap(classObjectCounterMap, instanceClassId, true)
                    MonitorLog.e(
                        TAG, "suspect leak! bitmap name: ${instance.instanceClassName}" + " width: ${width} height:${height}"
                    )
                    if(objectCounter.leakCnt <= SAME_CLASS_LEAK_OBJECT_PATH_THRESHOLD) {
                        mLeakingObjectIds.add(instance.objectId)
            ...
            if(nativeAllocationHeapClass?.objectId == superId1 || nativeAllocationThunkHeapClass?.objectId == superId1 || windowClass?.objectId == superId1) {
        ...
        while (primitiveArrayIterator.hasNext()) {
            ...
            if(arraySize >= DEFAULT_BIG_PRIMITIVE_ARRAY) {
                ...
                MonitorLog.e(
                    OOM_ANALYSIS_TAG,
                    "uspect leak! primitive arrayName:" + arrayName + " size:" + arraySize + " typeName:" + typeName + ", objectId:" + (primitiveArray.objectId and 0xffffffffL) + ", toString:" + primitiveArray.toString()
                )
        ...
        while (objectArrayIterator.hasNext()) {
            ...
            if(arraySize >= DEFAULT_BIG_OBJECT_ARRAY) {
                ...
                MonitorLog.i(
                    OOM_ANALYSIS_TAG, "object arrayName:" + arrayName + " objectId:" + objectArray.objectId
                )
    ...
    private fun findPathsToGcRoot() {
        ...
        val heapAnalyzer = HeapAnalyzer(OnAnalysisProgressListener { step: OnAnalysisProgressListener.Step ->
            MonitorLog.i(TAG, "step:" + step.name + ", leaking obj size:" + mLeakingObjectIds.size)
        })
        
        val findLeakInput = FindLeakInput(
            mHeapGraph, AndroidReferenceMatchers.appDefaults, false, mutableListOf()
        )
        ...
        MonitorLog.i(
            OOM_ANALYSIS_TAG, "---------------------------Application Leak---------------------------------------"
        ) //填充application leak
        MonitorLog.i(OOM_ANALYSIS_TAG, "ApplicationLeak size:" + applicationLeaks.size)
        for (applicationLeak in applicationLeaks) {
            MonitorLog.i(
                OOM_ANALYSIS_TAG, "shortDescription:" + applicationLeak.shortDescription + ", signature:" + applicationLeak.signature + " same leak size:" + applicationLeak.leakTraces.size
            )
            ...
            MonitorLog.i(
                OOM_ANALYSIS_TAG,
                "GC Root:" + gcRoot + ", leakObjClazz:" + leakTraceObject.className + ", leakObjType:" + leakTraceObject.typeName + ", labels:" + labels.contentToString() + ", leaking reason:" + leakTraceObject.leakingStatusReason + ", leaking obj:" + (leakTraceObject.objectId and 0xffffffffL)
            )
            ...
            for (reference in referencePath) {
                ...
                MonitorLog.i(
                    OOM_ANALYSIS_TAG,
                    "clazz:" + clazz + ", referenceName:" + referenceName + ", referenceDisplayName:" + referenceDisplayName + ", referenceGenericName:" + referenceGenericName + ", referenceType:" + referenceType + ", declaredClassName:" + declaredClassName
                )
                
                val leakPathItem = HeapReport.GCPath.PathItem().apply {
                    this.reference = if(referenceDisplayName.startsWith("["))  //数组类型[]
                        clazz
                    else "$clazz.$referenceDisplayName"
        ...
        MonitorLog.i(OOM_ANALYSIS_TAG, "----------------------------Library Leak--------------------------------------"); //填充library leak
        MonitorLog.i(OOM_ANALYSIS_TAG, "LibraryLeak size:" + libraryLeaks.size)
        for (libraryLeak in libraryLeaks) {
            MonitorLog.i(
                OOM_ANALYSIS_TAG, "description:" + libraryLeak.description + ", shortDescription:" + libraryLeak.shortDescription + ", pattern:" + libraryLeak.pattern.toString()
            )
            ...
            MonitorLog.i(
                OOM_ANALYSIS_TAG, "GC Root:" + gcRoot + ", leakClazz:" + leakTraceObject.className + ", labels:" + labels.contentToString() + ", leaking reason:" + leakTraceObject.leakingStatusReason
            )
            ...
            for (reference in referencePath) {
                ...
                val leakPathItem = HeapReport.GCPath.PathItem().apply {
                    this.reference = if(referenceDisplayName.startsWith("[")) clazz
                    else  //数组类型[]
        ...
        MonitorLog.i(
            OOM_ANALYSIS_TAG, "findPathsToGcRoot cost time: " + (endTime - startTime).toFloat() / 1000
        )
    }
    ...
    private fun updateClassObjectCounterMap(
        classObCountMap: MutableMap<Long, ObjectCounter>, instanceClassId: Long, isLeak: Boolean
    ): ObjectCounter {
